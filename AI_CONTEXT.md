...
* **2026-05-31 (Invitation Links, Layout Adjustment, & Query Parallelization):** Added the public Web App URL `https://splitwise-3fcdd.web.app` to all email and WhatsApp invitation templates in `InviteHelper`. Rearranged the `AddExpenseScreen` UI to vertically stack the Description and Amount input fields (rather than placing them horizontally side-by-side) and added "Amount" with the local currency symbol inside the hint text. Optimized database queries in `AppProvider.loadDashboardData` by executing Firestore network calls in parallel using `Future.wait`, resolving slow buffering times when creating groups and adding expenses. Verified all 17 tests pass successfully and deployed to Firebase Hosting.
* **2026-05-31 (Duplicate Friend & Member Selection Prevention):** Fixed a duplicate friend bug in `AppProvider.inviteUser` by searching the database for an existing user/placeholder with the same email before creating a new placeholder profile. Upgraded `DbService.getFriends` to automatically deduplicate active friend records by email (preferring real user accounts over placeholders) in both Firebase and Mock modes. Updated the friends selector bottom sheets in both `CreateGroupScreen` and `GroupDetailScreen` to filter out any friends whose emails are already added or staged, preventing duplicate members from being selected. Verified all 17 tests pass successfully and deployed web updates.
* **2026-05-31 (Invite Dialog Local Loading Indicators):** Integrated a `StatefulBuilder` and an active loading state (`isSavingLocal`) inside the "Invite Friend" dialogs of both `CreateGroupScreen` and `GroupDetailScreen`. When clicking "Invite & Add", the name input field is replaced by a centered `CircularProgressIndicator` and action buttons are hidden to prevent double submissions and clearly show the processing state in the foreground rather than behind the dialog. Verified all 17 tests pass successfully and deployed web updates.
